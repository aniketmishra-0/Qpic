...
"""PyInstaller spec for Qpic desktop build.

Builds a windowed (no-console) app that bundles the FastAPI backend, the static
web UI and the Python deps. Produces a ``.app`` on macOS and a folder/exe on
Windows.
...
# Bundle a self-contained Tesseract if it was vendored (scripts/vendor_tesseract.py).
# This puts the binary + its libs + tessdata at <bundle>/tesseract/, where
# app.services.detector.tesseract_locator looks for it at runtime. Without this
# the packaged app has no OCR unless the end user separately installs Tesseract.
if os.path.isdir("vendor/tesseract"):
    datas += [("vendor/tesseract", "tesseract")]
    print("desktop.spec: bundling vendored Tesseract from vendor/tesseract")
else:
    print(
        "desktop.spec: WARNING no vendor/tesseract found — OCR will require a "
